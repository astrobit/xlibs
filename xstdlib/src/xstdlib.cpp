...
// = sign is optional (-data=10, -data = 10, -data 10 will all be read)
	...
	if (i_iArgC > 1 && i_lpszOption != nullptr && i_lpszArgV != nullptr)
	{
		size_t iOption_Len = strlen(i_lpszOption);
		for (uiI = 1; uiI < (unsigned int)i_iArgC && lpszRet == nullptr; uiI++) // start from 1 - 0 is the command
		{
...
// = sign is optional (-data=10, -data = 10, -data 10 will all be read)
	...
	if (i_lpszDefault_Value != nullptr)
	{
...
std::string xParse_Command_Line_String(int iArgC,const char * lpszArgV[],const std::string & i_szOption, const std::string &i_szDefault_Value)
{ // if the command line option is say "data", this is what input option should be
// e.g. command data = 10
// if option is -data, (e.g. command -data 10) then pass "-data" as option input
// = sign is optional (-data=10, -data = 10, -data 10 will all be read)
	...
	unsigned int	uiIdx = 0;
	char chTerminator = 0;
	if (!i_szDefault_Value.empty())
	{
		o_szRet = i_szDefault_Value;
	}
	const char * lpszCursor = xParse_Command_Line_Data_Ptr(iArgC,lpszArgV,i_szOption.c_str());
	if (lpszCursor)
	{
		if (lpszCursor[0] == '\"' || lpszCursor[0] == '\'')
		{
		...
		if (lpszCursor[0] && lpszCursor[0] != 0 && lpszCursor[0] != chTerminator)
			o_szRet.clear();

		while (lpszCursor[0] && lpszCursor[0] != 0 && lpszCursor[0] != chTerminator)
		{
			o_szRet.push_back(lpszCursor[0]);
			lpszCursor++;
		}
	}
	return o_szRet;
...
double xGet_Element_Number(const char * i_lpszNuclide)
{
	double dZ;
	...
	if (strcmp(lpszElem,"H") == 0)
		dZ = 1.0;
	else if (strcmp(lpszElem,"HE") == 0 || strcmp(lpszElem,"He") == 0)
		dZ = 2.0;
	else if (strcmp(lpszElem,"LI") == 0 || strcmp(lpszElem,"Li") == 0)
		dZ = 3.0;
	else if (strcmp(lpszElem,"BE") == 0 || strcmp(lpszElem,"Be") == 0)
		dZ = 4.0;
